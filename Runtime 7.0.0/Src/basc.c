...
void verb_serialSet()								//03.04.02 GH - for newSerialManager
{
   ...
   StrToLower(in,in);
       ...
       if (StrCompare(in,"ir")==0)
       {
            ...
            if (i==0)
            {
            err=SrmControl(serRef, srmCtlIrDADisable,0,0);
            err=SrmControl(serRef, srmCtlRxDisable,0,0);
            }
            else if (i==1) //receive IR
            {
            err=SrmControl(serRef, srmCtlIrDAEnable,0,0);
            err=SrmControl(serRef, srmCtlRxEnable,0,0);
            }
	   ...
	   if (StrCompare(in,"break")==0)
	    {
	        if (i==0)
	        {
	        err=SrmControl(serRef, srmCtlStopBreak,0,0);
	        }
	        else
	        {
	        err=SrmControl(serRef, srmCtlStartBreak,0,0);
	        }
	        goto processed;
	    }
